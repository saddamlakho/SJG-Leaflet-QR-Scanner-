...
export default function UpdateRecord() {
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      ...
      const trimmedBase64 = base64String.replace(/^data:application\/pdf;base64,/, '');
  ...
  const updateData = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setErrorMessage(result.message || 'Error updating record');
      }
    } catch (error) {
      setErrorMessage(`Error updating record: ${error.message}`);
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="max-w-md w-full bg-white p-6 rounded-lg shadow-lg relative">
            ...
            <form onSubmit={updateData} className="space-y-4">
              <div className="flex flex-col">
                <label htmlFor="ID" className="font-medium text-lg mb-1">ID</label>
                <input
                  type="text"
                  id="ID"
                  value={formData.ID}
              ...
              <div className="flex flex-col">
                <label htmlFor="qr_code" className="font-medium text-lg mb-1">Leaflet PDF</label>
                <input
                  type="file"
                  accept=".pdf"
              ...
              <button
                ...
                {loading ? 'Submitting...' : 'Update Record'}
              </button>
            </form>

            <button onClick={() => setIsModalOpen(false)} className="absolute top-2 right-2 text-gray-500 hover:text-gray-700"> X </button>
          </div>
        </div>
      )}
    </div>
